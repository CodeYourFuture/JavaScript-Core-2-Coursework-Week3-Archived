...
function showMovies() {
  let divElement = document.querySelector("#all-movies");
  
  movies.forEach((movie) => {
    let liElement = document.createElement("li");
    liElement.innerText = `${movie.title},  ${movie.director},  ${movie.type}  ${movie.haveWatched ? "✅" : "❗️"
    }`;
...
function addMovie(movie, callback) {
  setTimeout(function () {
    // we pretend that pushing movie takes time
    movies.push(movie);
    callback();
  }, 2000);
